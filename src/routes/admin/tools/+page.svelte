<script lang="ts">
  ...
  async function updateTool(tool: AdminTool) {
    const response = await adminFetch(`/api/admin/tools/${tool.id}`, {
      method: 'PATCH',
    ...
    message = response.ok ? 'Saved ✓' : response.status === 401 ? 'Token invalid — re-enter it above.' : 'Save failed';
    await loadTools();
  }
  ...
  <main class="mx-auto max-w-7xl px-6 py-8">
    <div class="mb-6 flex items-center justify-between">
      ...
      <div class="flex gap-2">
        ...
        <a class="rounded border px-4 py-2" href="/admin/pipeline">Pipeline</a>
      </div>
    </div>
    ...
    {#if authError}
      <p class="mb-4 rounded border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
        The stored token is invalid. Re-enter it above.
      </p>
    {:else if loading}
      <p>Loading tools...</p>
    {:else}
      <div class="overflow-x-auto rounded-lg border bg-white dark:bg-slate-800">
        <table class="w-full min-w-[900px] text-left text-sm">
          <thead class="border-b bg-slate-50 dark:bg-slate-700">
            <tr>
          ...
          <tbody>
            {#each tools as tool}
              <tr class="border-b align-top">
                <td class="p-3">
                  <a class="font-semibold text-blue-700 dark:text-blue-400" href="/tools/{tool.slug}">{tool.name}</a>
                ...
                <td class="p-3">
                  <button class="rounded bg-slate-950 px-3 py-2 text-white dark:bg-blue-600" onclick={() => updateTool(tool)}>Save</button>
                </td>
              </tr>
            {/each}
          </tbody>
