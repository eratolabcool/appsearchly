/**
 * [INPUT]: 依赖 $lib/server/admin-auth 的 isAdminAuthorized，依赖 $lib/server/cron 的 runDailyCron/runWeeklyArticleCron
 * [OUTPUT]: POST /api/admin/cron — 手动触发定时任务（job=daily|weekly），返回执行报告
...
export const POST: RequestHandler = async ({ platform, request }) => {
  ...
  const job = body?.job === 'weekly' ? 'weekly' : 'daily';
